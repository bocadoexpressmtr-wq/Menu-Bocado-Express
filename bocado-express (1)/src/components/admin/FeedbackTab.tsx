        ...
        <div className="grid gap-4">
          {feedbacks.map((item) => (
            <div 
              key={item.id} 
              className={cn(
                "bg-white p-6 rounded-[2rem] border transition-all group relative",
                item.status === 'unread' ? "border-stone-900 shadow-lg" : "border-stone-100 shadow-sm opacity-80"
              )}
            >
              <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                <div className="space-y-4 flex-1">
                  <div className="flex items-center gap-3">
                    ...
                    {item.status === 'unread' && (
                      <span className="bg-stone-900 text-white text-[10px] font-black px-2 py-0.5 rounded-full uppercase tracking-widest animate-pulse">
                        Nuevo
                      </span>
                    )}
                  </div>

                  <p className="text-stone-800 font-medium leading-relaxed text-lg">
                    "{item.message}"
                  </p>
                  ...
                  <div className="flex flex-wrap items-center gap-6 pt-2">
                    <div className="flex items-center gap-2 text-stone-500">
                      ...
                      <span className="text-xs font-bold uppercase tracking-tight">{item.customerName}</span>
                    </div>
                    {item.customerPhone && (
                      <div className="flex items-center gap-2 text-stone-500">
                        <Phone size={16} className="text-stone-400" />
                        <span className="text-xs font-bold">{item.customerPhone}</span>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="flex md:flex-col gap-2 shrink-0">
                  ...
                  <button
                    ...
                    className="p-3 bg-red-50 text-red-500 rounded-2xl hover:bg-red-100 transition-all flex items-center gap-2 font-bold text-xs uppercase tracking-widest"
                  >
                    <Trash2 size={18} />
                    Eliminar
                  </button>
